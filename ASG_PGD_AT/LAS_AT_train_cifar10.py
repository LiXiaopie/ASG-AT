...
def get_args():
    ...
    parser.add_argument('--target_model_lr', default=0.001, type=float, help='learning rate')
...
from collections import OrderedDict
target_model_checkpoint = torch.load("../output/model.pth")
new_state_dict = OrderedDict()
for k, v in target_model_checkpoint.items():
        name = k[7:]  # remove `module.`
#        name = k
        new_state_dict[name] = v
target_model.load_state_dict(new_state_dict)
#target_model.load_state_dict(target_model_checkpoint)
start_peoch = 0
